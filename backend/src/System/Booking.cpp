...
BookingSystem::BookingSystem(const std::string& username, const std::string& password) : System(username, password),
...
bool BookingSystem::login(){
    ...
    if(handle){
        ...
        curl_easy_setopt(handle, CURLOPT_URL, "https://sts.cuhk.edu.cn/adfs/oauth2/authorize?response_type=code&client_id=caf5aded-3f28-4b64-b836-4451312e1ea3&redirect_uri=https://booking.cuhk.edu.cn/sso/code&client-request-id=f8739f9e-124f-4096-8b34-0140020000bb");
        CURLcode res;
...
FieldEvent BookingSystem::retrieve_field_info(const std::string& field_name, const std::string& start_time, const std::string& end_time){
    ...
    // URL编码参数
    char* escaped_field_id = curl_easy_escape(handle, field_id.c_str(), static_cast<int>(field_id.length()));
    char* escaped_start_time = curl_easy_escape(handle, start_time.c_str(), static_cast<int>(start_time.length()));
    char* escaped_end_time = curl_easy_escape(handle, end_time.c_str(), static_cast<int>(end_time.length()));
...
std::map<std::string, std::string> BookingSystem::_get_info(
    ...
    char* escaped_start_time = curl_easy_escape(handle, start_time.c_str(), static_cast<int>(start_time.length()));
    char* escaped_end_time = curl_easy_escape(handle, end_time.c_str(), static_cast<int>(end_time.length()));
    char* escaped_placeID = curl_easy_escape(handle, placeID.c_str(), static_cast<int>(placeID.length()));
...
) const
{
    ...
    // URL编码所有参数值
    char* escaped_placeID = curl_easy_escape(handle, placeID.c_str(), static_cast<int>(placeID.length()));
    char* escaped_reason = curl_easy_escape(handle, booker.reason.c_str(), static_cast<int>(booker.reason.length()));
    char* escaped_field_id = curl_easy_escape(handle, field_event.field_id.c_str(), static_cast<int>(field_event.field_id.length()));
    char* escaped_telephone = curl_easy_escape(handle, booker.telephone.c_str(), static_cast<int>(booker.telephone.length()));
    char* escaped_start_time = curl_easy_escape(handle, start_time.c_str(), static_cast<int>(start_time.length()));
    char* escaped_end_time = curl_easy_escape(handle, end_time.c_str(), static_cast<int>(end_time.length()));
    char* escaped_details = curl_easy_escape(handle, booker.details.c_str(), static_cast<int>(booker.details.length()));

    // 对info中的值进行编码
    std::map<std::string, std::string> escaped_info;
    for(const auto& pair : info) {
        char* escaped_value = curl_easy_escape(handle, pair.second.c_str(), static_cast<int>(pair.second.length()));
